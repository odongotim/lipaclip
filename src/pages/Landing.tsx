...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900">
      {/* Navbar */}
      <nav className="flex items-center justify-between px-8 py-5 border-b border-stone-200 sticky top-0 z-30 bg-stone-50/90 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <img src="src/assets/logo.jpg" alt="LipaClip" className="w-10 h-10 rounded-full" />
          <span className="font-display text-amber-600 text-2xl font-bold tracking-tight">Lipa<span className="text-stone-900">Clip</span></span>
        </div>
        <div className="flex items-center gap-4">
          <Link to="/login" className="text-stone-600 hover:text-stone-900 text-sm transition-colors">Login</Link>
          <Link to="/signup" className="gold-shimmer bg-amber-600 hover:bg-amber-700 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors">Get Started</Link>
        </div>
      </nav>

      {/* Hero */}
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-amber-50 blur-[120px]" aria-hidden="true" />

        <div className="relative max-w-6xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-amber-600 text-sm font-semibold tracking-wide uppercase mb-4"></p>
            <h1 className="font-display text-5xl md:text-6xl font-bold leading-[1.05] mb-6 tracking-tight">
              Clip it. Post it.<br /><span className="text-amber-600">Get paid in shillings.</span>
            </h1>
            ...
            <div className="flex gap-4 flex-wrap">
              <Link to="/signup?role=brand" className="gold-shimmer bg-amber-600 hover:bg-amber-700 text-white font-bold px-8 py-3 rounded-xl transition-colors text-sm shadow-lg shadow-amber-600/10">I'm a Brand</Link>
              <Link to="/signup?role=influencer" className="border border-amber-300 hover:border-amber-500 hover:bg-white text-amber-600 font-bold px-8 py-3 rounded-xl transition-colors text-sm">I'm a Creator</Link>
            </div>
          </div>
          ...
          */}
          <div className="relative aspect-[4/5] rounded-3xl border border-stone-200 overflow-hidden">
      ...
      {campaigns.length > 0 && (
        <section className="max-w-5xl mx-auto px-6 pb-16">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {campaigns.map(camp => (
              <div key={camp.id} className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
                ...
                <div className="w-full h-36 bg-amber-50 flex items-center justify-center overflow-hidden relative">
                  {camp.thumbnail_url
                    ? <img src={camp.thumbnail_url} alt={camp.title} className="w-full h-full object-cover" />
                    : <span className="text-stone-400 text-xs">No thumbnail yet</span>
                  }
                  ...
                  {camp.platforms && (
                    <div className="flex gap-1.5 flex-wrap mb-2">
                      {camp.platforms.map(p => (
                        <span key={p} className="text-[10px] uppercase tracking-wide bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">
                          {PLATFORM_LABELS[p] || p}
                        </span>
                      ))}
                    </div>
          ...
          <div className="text-center mt-6">
            <Link to="/signup?role=influencer" className="bg-amber-600 hover:bg-amber-700 text-white font-bold px-8 py-3 rounded-xl transition text-sm">Join and start earning</Link>
          </div>
        </section>
      )}
      ...
      <section className="grid grid-cols-3 gap-3 sm:gap-6 max-w-3xl mx-auto px-6 pb-16">
        {[{ label: 'Active Campaigns', value: '50+' }, { label: 'Creators Paid', value: 'UGX 2M+' }, { label: 'Verified Influencers', value: '100+' }].map(stat => (
          <div key={stat.label} className="bg-white border border-stone-200 rounded-2xl p-3 sm:p-6 text-center min-w-0">
            <div className="text-amber-600 text-xl sm:text-2xl md:text-3xl font-bold mb-1 leading-tight break-words">{stat.value}</div>
            <div className="text-stone-500 text-[10px] sm:text-xs leading-snug">{stat.label}</div>
          </div>
        ))}
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-24">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[{ step: '01', title: 'Brand Posts Campaign', desc: 'Set a budget, pick a campaign type and set the rate per 1,000 views.' }, { step: '02', title: 'Creator Makes Content', desc: 'Creators cut a video, post it on their own channel and submit the link.' }, { step: '03', title: 'Everyone Gets Paid', desc: 'Views are tracked, and payment lands in Mobile Money once verified.' }].map(item => (
            <div key={item.step} className="bg-white border border-stone-200 rounded-2xl p-6">
              <div className="text-amber-600/30 text-5xl font-bold mb-4">{item.step}</div>
      ...
      <footer className="border-t border-stone-200 px-8 py-6 text-center text-stone-400 text-sm">
