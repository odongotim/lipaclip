...
export default function NewCampaign() {
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('submit-pesapal-order', {
        body: { campaignId, amount: total, email: user.email },
      })

      if (fnError || !data?.redirect_url) {
        throw new Error(data?.error || fnError?.message || 'Could not start Pesapal checkout')
      }

      // Send the brand to Pesapal's hosted checkout page
    ...
    return (
      <div className="min-h-screen bg-[#0f0a06] flex">
        ...
        <main className="lg:ml-64 flex-1 p-6 pt-16 lg:pt-8">
          ...
          <p className="text-gray-400 text-sm mb-8">Pay securely via Pesapal to activate your campaign</p>
          <div className="max-w-lg">
            ...
            {/* Checkout preview */}
            <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-6 mb-6 flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-yellow-500/10 flex items-center justify-center flex-shrink-0"><svg className="w-6 h-6 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-9 4h16a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></div>
              <div>
                <h2 className="text-white font-bold text-lg">Pesapal Checkout</h2>
                <p className="text-gray-400 text-xs mt-0.5">Pay with card, mobile money, or bank. Pesapal will guide you through it.</p>
              </div>
            </div>
            ...
            {/* Info box */}
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 mb-6 flex items-start gap-2">
              <svg className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              <p className="text-blue-400 text-xs">You'll be redirected to Pesapal's secure checkout to pay by card or mobile money. Your campaign goes <strong>LIVE</strong> automatically the moment payment is confirmed.</p>
            </div>
