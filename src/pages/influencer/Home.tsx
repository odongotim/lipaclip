              ...
              <div key={camp.id} className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl overflow-hidden">
                <div className="w-full h-40 bg-yellow-500/10 flex items-center justify-center overflow-hidden relative">
                  {camp.thumbnail_url
                    ...
                    : <svg className="w-8 h-8 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.55-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.45.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                  }
                  ...
                  {camp.platforms && camp.platforms.length > 0 && (
                    <div className="flex gap-1 mb-3">
                      {camp.platforms.map(p => (
                        <span key={p} className="text-[10px] uppercase tracking-wide bg-yellow-500/10 text-yellow-400 px-1.5 py-0.5 rounded" title={p}>{PLATFORM_LABELS[p] || p}</span>
                      ))}
                    </div>
                  )}
