...
export default function InfluencerHome() {
  ...
  const loadData = async () => {
    ...
    const { data: camps } = await supabase.from('campaigns').select('*').eq('status', 'live').order('created_at', { ascending: false })
    if (camps) {
      // Only show campaigns that still have budget remaining
      setCampaigns(camps.filter((c: any) => (c.spent || 0) < c.budget))
    }

    // Get my submissions to know which campaigns I've already submitted to
  ...
  const handleSubmit = async (campaignId: string) => {
    ...
    await supabase.from('submissions').insert({
      campaign_id: campaignId, influencer_id: user.id,
      video_url: url, status: 'pending',
    })
